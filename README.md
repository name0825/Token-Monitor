...
- 트레이 메뉴: 지금 새로고침 / 클릭 통과 / 항상 위 / Claude 표시 / Codex 표시 / Windows 시작 시 실행 / 종료
- Windows 시작 시 자동 실행

## 데이터 소스
- **Claude**: `~/.claude/.credentials.json`을 읽기 전용으로 읽어 Claude OAuth usage API를 3분 간격으로 폴링합니다. 토큰을 갱신하거나 파일에 쓰지 않습니다. 실패 시 `claude` CLI를 실행해 `/usage` 화면을 스크래핑하는 방식으로 폴백합니다.
- **Codex**: `~/.codex/sessions` 아래 rollout 로그를 파싱합니다. 로그가 10분 넘게 오래되었거나 읽기에 실패하면 `codex` CLI를 실행해 `/status` 화면을 스크래핑하는 방식으로 폴백합니다(최소 10분 간격).
...
## 설정 파일
오버레이 위치, 클릭 통과 등 설정은 `%APPDATA%\TokenMonitor\settings.json`에 저장됩니다.
